...
@app.on_callback_query(filters.regex("RISHUmousAdmin") & ~BANNED_USERS)
async def RISHUmous_check(client, CallbackQuery):
    try:
        await CallbackQuery.answer(
...
@app.on_callback_query(filters.regex("RISHUPlaylists") & ~BANNED_USERS)
@languageCB
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                button = stream_markup(_, vidid, videoid, played, dur, chat_id)
        ...
        if await is_active_chat(chat_id):
            ...
            img = await get_thumb(vidid)
            button = stream_markup(_, vidid, chat_id, videoid, played, dur)
            run = await app.send_photo(
                original_chat_id,
                photo=img,
                caption=_["stream_1"].format(
